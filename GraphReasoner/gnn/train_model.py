...
class Trainer_KBQA(object):
    ...
    def load_ckpt(self, filename):
        ...
        model = self.model
        # Filter out parameters whose shapes don't match the current model to allow loading
        ...
        for k, v in model_state_dict.items():
            if k in cur_state:
                try:
                    ...
                    # if shape comparison fails, skip parameter
                    skipped.append(k)
            # else: parameter not present in current model, ignore

        if skipped:
            print("Skipping parameters with shape mismatch when loading checkpoint:")
            for s in skipped:
                print(" -", s)
