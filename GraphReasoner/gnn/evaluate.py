        ...
        for iteration in tqdm(range(num_epoch)):
            ...
            for batch_id in range(batch_size):
                ...
                if write_info:
                    ...
                    # sanitize question string: remove pad / special token markers and collapse spaces
                    qstr = tp_obj.get('question', '')
                    try:
                        for toks in ['<pad>', '<PAD>', '[PAD]', '[CLS]', '[SEP]', '<s>', '</s>']:
                            qstr = qstr.replace(toks, ' ')
                        # collapse multiple spaces
                        qstr = ' '.join(qstr.split())
                        tp_obj['question'] = qstr
                    except Exception:
                        pass
                    # write JSON with ensure_ascii=False so Chinese is human-readable
                    self.file_write.write(json.dumps(tp_obj, ensure_ascii=False) + "\n")
